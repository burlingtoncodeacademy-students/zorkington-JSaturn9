...
async function start() {
  ...
  let target = inputArray.slice(1/*-1 looks at the last input may need to change to this ie take small key = "key"*/).join(" ");//what you want to find in lookup table - takes input array, slices it at first index of array, then joins it back together

  if (action === "use") {//conditional statement for when action is use
    console.log(lookupItemTable[target].use());//this means use lookup table at the target and use use method on it
    start();
  } else if (action === "take") {
    if (lookupItemTable[target] instanceof Item) {//conditional for if an item exists in the room and is takeable
    ...
    start();
  }else if (action === "leave" || action === "exit") {//allows you to leave the game
    process.exit();
  } else {
